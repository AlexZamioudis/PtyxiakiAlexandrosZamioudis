# -*- coding: utf-8 -*-
"""
Created on Mon Apr 15 16:46:22 2019
...
if os.path.exists("1. iob\\deca.iob2"):
    os.remove("1. iob\\deca.iob2")

output = open("1. iob\\deca.iob2", "a", encoding="utf8")
...
def make_iob(txt, ents):
    ...
    try: #in order to handle the last case where list.index doesnt finds anything
        ...
        for i in range(0, len(ents)):
            ...
            entity_iob = ["I-Gene/Protein"] * len(entity_tokens)
        ...
        #the format is: token    IOB-Gene/Protein
...
with open("deca.txt", encoding="utf8") as fi:  
    ...
    while line:
        ...
        if len(line) != 1:
            annotations.append([ int(line[1]), int(line[2]), line[3]])
            ids.append(line[0])
        line = fi.readline()
...
for filename in os.listdir(dr):
    ...
    with open(filename, encoding="utf8") as fi:  
        ...
        text = ""
        while line:
            line = line.replace('\n',' ') #remove newlines but replace them with space to kepp the correct starts
            text += line
            
            line = fi.readline()
            
        text = ' '.join(text.split()) #remove tabs
