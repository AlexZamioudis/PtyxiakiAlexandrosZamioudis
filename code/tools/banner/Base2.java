...
public abstract class Base2 {
	...
	protected static HashMap<String, LinkedList<Base2.Tag>> getTags(BufferedReader tagFile) throws IOException
	{
	    HashMap<String, LinkedList<Base2.Tag>> tags = new HashMap<String, LinkedList<Base2.Tag>>();
	
	    String line = tagFile.readLine();
	    while (line != null)
	    {
	        String[] split = line.split(" |\\|");
	        LinkedList<Base2.Tag> tagList = tags.get(split[0]);
	        if (tagList == null)
	            tagList = new LinkedList<Base2.Tag>();
	        Base2.Tag tag = new Base2.Tag(split[1], Integer.parseInt(split[2]), Integer.parseInt(split[3]));
	        Iterator<Base2.Tag> tagIterator = tagList.iterator();
	        boolean add = true;
	        while (tagIterator.hasNext() && add)
	        {
	            Base2.Tag tag2 = tagIterator.next();
	...
	protected static HashMap<String, LinkedList<Base2.Tag>> getAlternateTags(BufferedReader tagFile) throws IOException
	{
	    HashMap<String, LinkedList<Base2.Tag>> tags = new HashMap<String, LinkedList<Base2.Tag>>();
	...
	protected static Sentence getSentence(String id, String sentenceText, Tokenizer tokenizer, HashMap<String, LinkedList<Base2.Tag>> tags)
	{
	    Sentence sentence = new Sentence(id, sentenceText);
	    tokenizer.tokenize(sentence);
	    List<Token> tokens = sentence.getTokens();
	    LinkedList<Base2.Tag> tagList = tags.get(id);
	    if (tagList != null)
			for (Base2.Tag tag : tagList)
			{
	        	...
	        	assert end >= start;
				sentence.addOrMergeMention(new Mention(sentence, MentionType.getType(type), start, end + 1));
			}
	    return sentence;
	}

	protected static Set<Mention> getMentions(Sentence sentence, HashMap<String, LinkedList<Base2.Tag>> tags)
	{
		Set<Mention> mentions = new HashSet<Mention>();
	    List<Token> tokens = sentence.getTokens();
	    LinkedList<Base2.Tag> tagList = tags.get(sentence.getTag());
	    if (tagList != null)
			for (Base2.Tag tag : tagList)
			{
	        	int start = getTokenIndex(tokens, tag.start);
	...
	protected static int checkTokenBoundaries(Sentence sentence, Tokenizer tokenizer, HashMap<String, LinkedList<Base2.Tag>> tags)
	{
	    List<Token> tokens = sentence.getTokens();
	    LinkedList<Base2.Tag> tagList = tags.get(sentence.getTag());
	    int count = 0;
		...
		for (int i = 0; i < tokens.size(); i++) {
			...
			if (mention != null && i == mention.getStart()) {
				...
				mentionOutputFile.print("|");
                                mentionOutputFile.print(mention.getType());
                                mentionOutputFile.print("|");
